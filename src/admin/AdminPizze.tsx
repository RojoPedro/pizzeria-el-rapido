        ...
        <div className="bg-white p-6 rounded-lg shadow-sm border border-gray-200">
          ...
          <form onSubmit={handleSubmit} className="space-y-4">
            ...
            <div className="border-t border-gray-100 pt-4">
              <label className="block text-sm font-medium text-gray-700 mb-2">Ingredienti</label>
              <div className="grid grid-cols-2 md:grid-cols-4 gap-2 max-h-48 overflow-y-auto p-2 border border-gray-200 rounded-md">
        ...
        <table className="min-w-full divide-y divide-gray-200">
          <thead className="bg-gray-50">
            <tr>
              ...
              <th className="px-6 py-3 text-left text-xs font-medium text-gray-500 uppercase tracking-wider">Visibile</th>
              <th className="px-6 py-3 text-left text-xs font-medium text-gray-500 uppercase tracking-wider">Prezzo Norm.</th>
          ...
          <tbody className="bg-white divide-y divide-gray-200">
            {pizze.map(pizza => (
              <tr key={pizza.id}>
                <td className="px-6 py-4 whitespace-nowrap text-sm font-medium text-gray-900">{pizza.nome}</td>
                <td className="px-6 py-4 whitespace-nowrap text-sm text-gray-500">
                  {pizza.is_visible ? <Check className="w-5 h-5 text-green-500" /> : <X className="w-5 h-5 text-red-500" />}
                </td>
                <td className="px-6 py-4 whitespace-nowrap text-sm text-gray-500">€ {parseFloat(pizza.prezzi.normale).toFixed(2)}</td>
                <td className="px-6 py-4 text-sm text-gray-500 max-w-[200px] truncate" title={pizza.ingredienti.join(', ')}>
